...
class MPDTEApp(ctk.CTk):
    ...
    def _build_predict_tab(self):
        ...
        ctk.CTkLabel(header_row, text="⚡ Colleges Missed by Small Margin",
                      font=ctk.CTkFont(size=12, weight="bold"),
    ...
    def _build_results_tab(self):
        ...
        for opt in ["All", "Guaranteed", "Very Safe", "Safe", "Possible"]:
            ctk.CTkRadioButton(
        ...
        self.results_table = self._make_treeview(
            ...
            columns=["College", "Branch", "Type", "Open Rank", "Close Rank",
                      "Category", "FW", "Prob %", "Risk Level", "Explanation"],
            widths=[220, 200, 70, 90, 90, 120, 40, 70, 110, 200],
        )
    ...
    def _make_treeview(self, parent, columns, widths):
        ...
        tree.tag_configure("guaranteed", background="#003300", foreground="#00ff88")
        tree.tag_configure("very_safe", background="#1a3300", foreground="#7CB342")
        tree.tag_configure("safe", background="#332200", foreground="#FFD600")
        tree.tag_configure("possible", background="#331500", foreground="#FF6D00")
        tree.tag_configure("difficult", background="#330000", foreground="#D32F2F")
        tree.tag_configure("unlikely", background="#1a001a", foreground="#880E4F")
    ...
    def _build_strategy_tab(self):
        ...
        self.strategy_table = self._make_treeview(
            tab,
            columns=["#", "College", "Branch", "Type", "Close Rank", "Prob %", "Risk", "Score"],
            widths=[40, 250, 200, 80, 100, 70, 120, 70],
        )
    ...
    def _display_results(self, summary):
        ...
        prob_sum = summary["probability_summary"]

        # Update summary labels
        self.result_summary_labels["Total"].configure(text=str(summary["total_eligible_colleges"]))
        self.result_summary_labels["Guaranteed"].configure(text=str(prob_sum["guaranteed"]))
        self.result_summary_labels["Very Safe"].configure(text=str(prob_sum["very_safe"]))
        self.result_summary_labels["Safe"].configure(text=str(prob_sum["safe"]))
        self.result_summary_labels["Possible"].configure(text=str(prob_sum["possible"]))
        self.result_summary_labels["Difficult"].configure(text=str(prob_sum["difficult"]))
        ...
        self.predict_status.configure(
            text=f"✅ Analysis complete! Found {len(all_r)} options across {summary['total_eligible_colleges']} colleges.",
            text_color=COLORS["accent_green"]
        )
    ...
    def _fill_results_table(self, data):
        ...
        if sort_by == "Closing Rank":
            ...
            data = sorted(data, key=lambda x: -x.get("probability", 0))
        ...
        filter_risk = filter_map.get(filter_val)
    ...
    def _generate_strategy(self):
        ...
        risk_tags = {"Almost Guaranteed": "guaranteed", "Very Safe": "very_safe",
                      "Safe": "safe", "Possible": "possible"}
    ...
    def _run_simulator(self):
        ...
        parts = []
        for rank, cnt in sorted(result.items()):
            parts.append(f"Rank {rank:,} → {cnt:,} options")
